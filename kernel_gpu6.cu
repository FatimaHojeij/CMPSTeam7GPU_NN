...
__global__ void spmspm(COOMatrix *result, CSRMatrix A, CSCMatrix B, float bias, unsigned int* nnz_out) {
    ...
    if(threadIdx.x == 0 && threadIdx.y == 0){
		...
		for(unsigned int offset = 0;offset<nnz_s;++offset){
        	result->rowIdxs[nnzIndxTemp + offset] = rowIdxs_s[offset];
        	result->colIdxs[nnzIndxTemp + offset] = colIdxs_s[offset];
			result->values[nnzIndxTemp + offset] = values_s[offset];
		}
